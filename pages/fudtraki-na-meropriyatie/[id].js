...
import { BUCKET_ROUTE } from "../../const/routes";

import data from "../../public/data/foodtrucks/catalog.json";
...
const FoodtruckDetails = ({ foodtrucks }) => {
    ...
    return (
        <>
            ...
            <section className={styles.section}>
                <Breadcrumbs caption={foodtruck.caption} />{" "}
                <h1 className={`${styles.title} stn-title`}>
                    {foodtruck.caption}{" "}
                </h1>
                <Swiper
...
FoodtruckDetails.getInitialProps = ({ query }) => {
    return {
        foodtrucks: data.catalog.filter(
